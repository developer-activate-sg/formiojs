...
var _wizardTestFormWithNestedComponents = _interopRequireDefault(require("../test/forms/wizardTestFormWithNestedComponents"));
...
var _wizardWithPrefixComps = _interopRequireDefault(require("../test/forms/wizardWithPrefixComps"));
var _wizardPermission = _interopRequireDefault(require("../test/forms/wizardPermission"));
...
describe('Wizard tests', function () {
  it('Should recalculate values for components with "allow override" after wizard is canceled', function (done) {
    ...
    _Formio["default"].createForm(formElement, _formsWithAllowOverrideComps["default"].wizard).then(function (form) {
      ...
      var radioTrigger = form.getComponent('radio1');
      _powerAssert["default"].equal(number.dataValue, number.emptyValue);
      _powerAssert["default"].equal(textField.dataValue, textField.emptyValue);
      _powerAssert["default"].equal(textArea.dataValue, textArea.emptyValue);
  ...
  it('Should execute form controller', function (done) {
    ...
    form.display = 'wizard';
    _Formio["default"].createForm(form).then(function (form) {
      setTimeout(function () {
        var textField = form.getComponent('textField');
  ...
  it('Should check correctly Permissions and disabled sumbit button', function (done) {
    ...
    wizard.setForm(_wizardPermission["default"]).then(function () {
      wizard.form.disableWizardSubmit = true;
      wizard.redraw();
      var btn = wizard.element.querySelector('.btn-wizard-nav-submit');
      _powerAssert["default"].equal(btn.disabled, true);
      done();
    })["catch"](function (err) {
      return done(err);
    });
  ...
  it('Should render values for prefix Components', function (done) {
    ...
    var form = _lodash["default"].cloneDeep(_wizardWithPrefixComps["default"].form);
    wizard.setForm(form).then(function () {
      var clickWizardBtn = function clickWizardBtn(pathPart, clickError) {
        var btn = _lodash["default"].get(wizard.refs, clickError ? pathPart : "".concat(wizard.wizardKey, "-").concat(pathPart));
        var clickEvent = new Event('click');
      ...
      var checkValues = function checkValues() {
        wizard.refs["wizard-".concat(wizard.id)].querySelectorAll('input').forEach(function (element, i) {
          switch (i) {
            case 0:
              _powerAssert["default"].equal(element.value, 'prefix', 'Should render value');
              break;
            case 1:
              _powerAssert["default"].equal(element.value, "page".concat(wizard.page + 1), 'Should render value');
              break;
            case 2:
              _powerAssert["default"].equal(element.value, 'suffix', 'Should render value');
              break;
          }
        });
      };
      wizard.submission = _lodash["default"].cloneDeep(_wizardWithPrefixComps["default"].submission);
      setTimeout(function () {
        checkPage(0);
        checkValues();
        clickWizardBtn('next');
  ...
  it('Should NOT navigate to next page if it contains invalid nested component', function (done) {
    var formElement = document.createElement('div');
    var wizard = new _Wizard["default"](formElement);
    var form = _lodash["default"].cloneDeep(_wizardTestFormWithNestedComponents["default"].form);
    wizard.setForm(form).then(function () {
      var checkPage = function checkPage(pageNumber) {
        _powerAssert["default"].equal(wizard.page, pageNumber, "Should open wizard page ".concat(pageNumber + 1));
      };
      ...
      wizard.nextPage();
      setTimeout(function () {
        var errors = wizard.errors;
        checkPage(0);
        (0, _powerAssert["default"])(errors.length > 0, 'Must err before next page');
        _powerAssert["default"].equal(errors[0].message, 'Required Component is required');
        done();
      }, 200);
    })["catch"](function (err) {
      return done(err);
  ...
  it('Should show tooltip for wizard pages', function (done) {
    ...
    wizardWithPageTooltip.setForm(_wizardWithTooltip["default"]).then(function () {
      ...
      setTimeout(function () {
        var tooltipText = wizardWithPageTooltip.element.querySelector('.tippy-content').textContent;
