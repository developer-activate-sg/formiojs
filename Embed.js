...
function embed() {
  ...
  if (thisScript) {
    ...
    query.script = query.script || "".concat(config.updatePath ? config.updatePath() : scriptSrc, "/formio.form.min.js");
    query.styles = query.styles || "".concat(config.updatePath ? config.updatePath() : scriptSrc, "/formio.form.min.css");
    var cdn = query.cdn || 'https://cdn.jsdelivr.net/npm';
    var resolveLibs = function resolveLibs(cdn) {
      var libs = {
        uswds: {
          fa: true,
      ...
      if (cdn instanceof _CDN.default) {
        ...
        libs.fontawesome.css = "".concat(url, "/font-awesome/").concat(cdn.getVersion('font-awesome'), "/css/font-awesome.min.css");
        libs.bootstrap.css = "".concat(url, "/bootstrap/").concat(cdn.getVersion('bootstrap'), "/css/bootstrap.min.css");
      }
      return libs;
    };
    ...
    // Create a loader
    addStyles("".concat(config.updatePath ? config.updatePath() : scriptSrc, "/formio.embed.min.css"));
    ...
    addScript(config.script, 'Formio', function (Formio) {
      ...
      if (config.premium) {
        ...
        addScript(config.premium.js, 'premium', function (premium) {
          debug('Using premium');
          Formio.use(premium);
          renderForm();
        });
