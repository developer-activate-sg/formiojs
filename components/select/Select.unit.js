...
describe('Select Component', function () {
  ...
  it('Should return plain text when csv option is provided', function () {
    return _harness["default"].testCreate(_Select["default"], _fixtures.comp1).then(function (component) {
      _powerAssert["default"].equal(component.getView('red', {
        csv: true
      }), 'Red');
    });
  });
  ...
  it('Should provide metadata.selectData for Select component pointed to a resource where value property is set to a field', function (done) {
    var form = _lodash["default"].cloneDeep(_fixtures.comp17);
    var testItems = [{
      textField: 'John'
    }, {
      textField: 'Mary'
    ...
    _Formio["default"].createForm(element, form).then(function (form) {
      var select = form.getComponent('select');
      select.setItems(testItems.map(function (item) {
        return {
          data: item
        };
      }));
      ...
      setTimeout(function () {
        ...
        submitBtn.dispatchEvent(clickEvent);
        setTimeout(function () {
          _powerAssert["default"].equal(_lodash["default"].isEqual(form.submission.metadata.selectData.select.data, testItems[0]), true);
          done();
        }, 200);
      }, 200);
    })["catch"](done);
  ...
  it('Should provide correct metadata.selectData for multiple Select', function (done) {
    var form = _lodash["default"].cloneDeep(_fixtures.comp20);
    var element = document.createElement('div');
    _Formio["default"].createForm(element, form).then(function (form) {
      var select = form.getComponent('select');
      var values = ['apple', 'orange'];
      ...
      setTimeout(function () {
        ...
        submitBtn.dispatchEvent(clickEvent);
        setTimeout(function () {
          var metadata = form.submission.metadata.selectData.select;
          _powerAssert["default"].equal(_lodash["default"].keys(metadata).length, 2);
          values.forEach(function (value) {
            _powerAssert["default"].equal(_lodash["default"].find(select.component.data.values, {
              value: value
  ...
  it('Should provide correct metadata.selectData for HTML5 Select', function (done) {
    var element = document.createElement('div');
    _Formio["default"].createForm(element, _fixtures.comp21).then(function (form) {
      var select = form.getComponent('animals');
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          submitBtn.dispatchEvent(clickEvent);
          setTimeout(function () {
            var metadata = form.submission.metadata.selectData.animals2;
            _powerAssert["default"].equal(metadata.label, 'Dog');
            done();
          }, 200);
        }, 300);
      }, 200);
  ...
  it('Should provide correct metadata.selectData for HTML5 Select with default value', function (done) {
    var form = _lodash["default"].cloneDeep(_fixtures.comp22);
    var element = document.createElement('div');
    _Formio["default"].createForm(element, form).then(function (form) {
      var submit = form.getComponent('submit');
      var clickEvent = new Event('click');
      ...
      setTimeout(function () {
        var metadata = form.submission.metadata.selectData.select;
        _powerAssert["default"].equal(metadata.label, 'Label 1');
        done();
      }, 200);
    })["catch"](done);
  });
  it('Should provide correct metadata.selectData for ChoicesJS Select with default value', function (done) {
    var form = _lodash["default"].cloneDeep(_fixtures.comp22);
  ...
  it('Should set correct label from metadata for ChoicesJS Select with default value', function (done) {
    var form = _lodash["default"].cloneDeep(_fixtures.comp22);
    form.components[0].widget = 'choicesjs';
    var element = document.createElement('div');
    _Formio["default"].createForm(element, form).then(function (form) {
      ...
      setTimeout(function () {
        _powerAssert["default"].equal(select.templateData['value2'].label, 'Label 2');
        done();
      }, 200);
    })["catch"](done);
  });
  it('Should provide correct metadata.selectData for multiple Select with default value', function (done) {
    var form = _lodash["default"].cloneDeep(_fixtures.comp23);
    var element = document.createElement('div');
    _Formio["default"].createForm(element, form).then(function (form) {
      var submit = form.getComponent('submit');
      var clickEvent = new Event('click');
      ...
      setTimeout(function () {
        var metadata = form.submission.metadata.selectData.select;
        _powerAssert["default"].deepEqual(metadata, {
          value1: {
            label: 'Label 1'
          },
          value3: {
  ...
  it('Should set correct label from metadata for multiple Select with default value', function (done) {
    var form = _lodash["default"].cloneDeep(_fixtures.comp23);
    var element = document.createElement('div');
    _Formio["default"].createForm(element, form).then(function (form) {
      var select = form.getComponent('select');
      ...
      setTimeout(function () {
        _powerAssert["default"].equal(select.templateData['value1'].label, 'Label 1');
        _powerAssert["default"].equal(select.templateData['value2'].label, 'Label 2');
        done();
      }, 200);
    })["catch"](done);
  ...
  it('OnBlur validation should work properly with Select component', function (done) {
    this.timeout(0);
    var element = document.createElement('div');
    _Formio["default"].createForm(element, _fixtures.comp19).then(function (form) {
      var select = form.components[0];
      select.setValue('banana');
      select.focusableElement.focus();
      select.pristine = false;
      setTimeout(function () {
        (0, _powerAssert["default"])(!select.error, 'Select should be valid while changing');
        select.focusableElement.dispatchEvent(new Event('blur'));
        setTimeout(function () {
          (0, _powerAssert["default"])(select.error, 'Should set error after Select component was blurred');
          done();
        }, 500);
      }, 200);
    })["catch"](done);
  ...
  it('Should escape special characters in regex search field', function (done) {
    ...
    _Formio["default"].createForm(element, form).then(function (form) {
      ...
      setTimeout(function () {
        ...
        searchField.value = '^$.*+?()[]{}|';
        searchField.dispatchEvent(keyupEvent);
        var spy = _sinon["default"].spy(_Formio["default"], 'makeRequest');
        setTimeout(function () {
          _powerAssert["default"].equal(spy.callCount, 1);
          var urlArg = spy.args[0][2];
          _powerAssert["default"].ok(urlArg && typeof urlArg === 'string' && urlArg.startsWith('http'), 'A URL should be passed as the third argument to "Formio.makeRequest()"');
          _powerAssert["default"].ok(urlArg.includes('__regex=%5C%5E%5C%24%5C.%5C*%5C%2B%5C%3F%5C(%5C)%5C%5B%5C%5D%5C%7B%5C%7D%5C%7C'), 'The URL should contain escaped and encoded search value regex');
          done();
        }, 500);
      }, 200);
    })["catch"](done);
...
describe('Select Component', function () {
  it('Select Component should work correctly with the values in the form of an array', function (done) {
    var form = _lodash["default"].cloneDeep(_fixtures.comp18);
    var testItems = [{
      textField: ['one', 'two']
    }, {
...
describe('Select Component with Entire Object Value Property', function () {
  ...
  it('Should provide correct items for Resource DataSrc Type and Entire Object Value Property', function (done) {
    ...
    _Formio["default"].createForm(element, form).then(function (form) {
      ...
      setTimeout(function () {
        ...
        submitBtn.dispatchEvent(clickEvent);
        setTimeout(function () {
          _powerAssert["default"].equal(_typeof(select.dataValue), 'object');
          done();
        }, 200);
      }, 200);
    })["catch"](done);
  ...
  it('Should provide correct html value for Resource DataSrc Type and Entire Object Value Property', function (done) {
    ...
    _Formio["default"].createForm(element, form).then(function (form) {
      ...
      var selectContainer = element.querySelector('[ref="selectContainer"]');
      _powerAssert["default"].notEqual(selectContainer, null);
      var options = selectContainer.childNodes;
      _powerAssert["default"].equal(options.length, 4);
      options.forEach(function (option) {
        _powerAssert["default"].notEqual(option.value, '[object Object]');
      });
      var value = {
        textField: 'Jone',
        number: 1
  ...
  it('Should set submission value for Resource DataSrc Type and Entire Object Value Property', function (done) {
    ...
    _Formio["default"].createForm(element, form).then(function (form) {
      ...
      setTimeout(function () {
        ...
        var selectContainer = element.querySelector('[ref="selectContainer"]');
        _powerAssert["default"].notEqual(selectContainer, null);
        _powerAssert["default"].notEqual(selectContainer.value, '');
        var options = selectContainer.childNodes;
        _powerAssert["default"].equal(options.length, 2);
        done();
      }, 1000);
    })["catch"](done);
  });
  it('Should get string representation of value for Resource DataSrc Type and Entire Object Value Property', function (done) {
    _harness["default"].testCreate(_Select["default"], _fixtures.comp15.components[0]).then(function (component) {
      var entireObject = {
        a: '1',
        b: '2'
      };
      var formattedValue = component.getView(entireObject);
      _powerAssert["default"].equal(formattedValue, JSON.stringify(entireObject));
      done();
    });
  });
});