...
describe('Tags Component', function () {
  ...
  it('Should show the specified delimiter when get value as string', function (done) {
    ...
    _Formio.default.createForm(element, form).then(function (form) {
      ...
      tags.setValue(value);
      setTimeout(function () {
        _powerAssert.default.deepEqual(tags.getValue(), value.join(tags.component.delimeter));
        _powerAssert.default.deepEqual(form.submission.data.tags, value);
        _powerAssert.default.equal(tags.dataValue, value);
        _powerAssert.default.equal(tags.getValueAsString(value), value.join("".concat(tags.component.delimeter, " ")));
        document.innerHTML = '';
        done();
      }, 200);
    }).catch(done);
