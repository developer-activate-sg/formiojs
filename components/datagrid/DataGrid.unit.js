...
describe('DataGrid calculated values', function () {
  it('Should allow override calculated value', function (done) {
    _Formio["default"].createForm(document.createElement('div'), _fixtures.withAllowCalculateOverride).then(function (form) {
      var select = form.getComponent('select');
      var dataGrid = form.getComponent('dataGrid');
      _powerAssert["default"].deepEqual(dataGrid.getValue(), [{
        firstName: '',
        lastName: ''
      }]);
      select.setValue('a', {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          var firstName = form.getComponent(['dataGrid', 0, 'firstName']);
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          setTimeout(function () {
            ...
            firstName.setValue('A f 1', {
              modified: true
            });
            setTimeout(function () {
              _powerAssert["default"].equal(select.getValue(), 'c');
              _powerAssert["default"].deepEqual(dataGrid.getValue(), [{
                firstName: 'A f 1',
                lastName: 'A l 1'
              }]);
  ...
  it('Should calculate value for several DataGrid components', function (done) {
    _Formio["default"].createForm(document.createElement('div'), _fixtures.twoWithAllowCalculatedOverride).then(function (form) {
      var select = form.getComponent('select');
      var dataGrid = form.getComponent('dataGrid');
      var dataGrid2 = form.getComponent('dataGrid2');
      _powerAssert["default"].deepEqual(dataGrid.getValue(), [{
        firstName: '',
        lastName: ''
      }]);
      _powerAssert["default"].deepEqual(dataGrid2.getValue(), [{
        firstName: '',
        lastName: ''
      }]);
      select.setValue('a', {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          var firstName2 = form.getComponent(['dataGrid2', 0, 'firstName']);
          firstName2.setValue('first name 2', {
            modified: true
          });
          select.setValue('c', {
            modified: true
...
describe('DataGrid Reorder', function () {
  it('Should display select components labels correctly on rows reorder', function (done) {
    _Formio["default"].createForm(document.createElement('div'), _fixtures.withReorder.form).then(function (form) {
      form.setSubmission(_fixtures.withReorder.submission).then(function () {
        var values = [{
          value: '11',
          label: 1
        }, {
        ...
        var dataGrid = form.getComponent('dataGrid');
        _lodash["default"].each(dataGrid.components, function (selectComp, ind) {
          var expectedValue = values[ind];
          _powerAssert["default"].equal(ind, selectComp.rowIndex);
          _powerAssert["default"].equal(selectComp.dataValue, expectedValue.value);
          _powerAssert["default"].equal(selectComp.templateData[expectedValue.value].data.number, expectedValue.label);
        });
        dataGrid.onReorder({
          dragInfo: {
            index: 4
        ...
        setTimeout(function () {
          ...
          _lodash["default"].each(dataGrid.components, function (selectComp, ind) {
            var expectedValue = values[ind];
            _powerAssert["default"].equal(ind, selectComp.rowIndex, 'Component index after reorder');
            _powerAssert["default"].equal(selectComp.dataValue, expectedValue.value, 'Component value after reorder');
            _powerAssert["default"].equal(selectComp.templateData[expectedValue.value].data.number, expectedValue.label, 'Component label value after reorder');
          });
          done();
        }, 600);
      });
